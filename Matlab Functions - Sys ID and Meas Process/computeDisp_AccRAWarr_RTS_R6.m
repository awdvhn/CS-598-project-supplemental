function [time, F_deNoise, x_acc, v_acc, a_acc, iFstart, iFend,...
    timeForce] = computeDisp_AccRAWarr_RTS_R6(timeRAW,...
    Fraw_Hammmer, aRAW_acc)
...
% a_acc_Detrend = filtfilt(b, a, detrend(aRAW_acc(I_ForceStart)));
a_acc_Detrend = detrend(aRAW_acc(I_ForceEnds));
...
% a_acc_Detrend(1:(iFend-iFstart)) = F_deNoise(1:(iFend-iFstart))/0.0245;
y = [a_acc_Detrend'; zeros(3, length(a_acc_Detrend))];
dt = mean(diff(time));
% F = [ 1,     dt,   0.5*dt^2,  1/6*dt^3,    0, 0, 0 ;... % [x] % The system matrix
%       0,     1,       dt,     0.5*dt^2,    0, 0, 0 ;... % [v]
%       0,     0,       1,         dt,       0, 0, 0 ;... % [a]
%       0,     0,       0,          1,       0, 0, 0 ;... % [da/dt]
%       B,     0,       0,          0,       A, 0, 0 ;... % [x_lowFreq(k+1)]
%       0,     B,       0,          0,       0, A, 0 ;... % [v_lowFreq(k+1)]
%       0,     0,       B,          0,       0, 0, A];    % [a_lowFreq(k+1)]
% 
% H = [0, 0, 1, dt, 0, 0, 0 ;... % Measured acceleration(k)
%      D, 0, 0,  0, C, 0, 0 ;... % x_lowFreq(k)
%      0, D, 0,  0, 0, C, 0 ;... % v_lowFreq(k)
%      0, 0, D,  0, 0, 0, C];    % a_lowFreq(k)
 F = [ 1,     dt,   0.5*dt^2,  0, 0, 0 ;... % [x] % The system matrix
      0,     1,       dt,      0, 0, 0 ;... % [v]
      0,     0,       1,       0, 0, 0 ;... % [a]
      B,     0,       0,       A, 0, 0 ;... % [x_lowFreq(k+1)]
      0,     B,       0,       0, A, 0 ;... % [v_lowFreq(k+1)]
      0,     0,       B,       0, 0, A];    % [a_lowFreq(k+1)]

H = [0, 0, 1, 0, 0, 0 ;... % Measured acceleration(k)
     D, 0, 0, C, 0, 0 ;... % x_lowFreq(k)
     0, D, 0, 0, C, 0 ;... % v_lowFreq(k)
     0, 0, D, 0, 0, C];    % a_lowFreq(k)
     
ss = size(F, 2); % State space size
os = size(y, 1); % observation space size
Q = 2 * 2e-4*eye(ss); % The system covariance
R = 10e-1*eye(os); % The observation covariance: Original
X0 = zeros(ss, 1); % the initial state (column) vector
X0(2) = trapz(timeForce,F_deNoise)/0.0245; 
X0(3) = - 0 * 0.2*max(F_deNoise)/0.0245; 
% X0(4) = - 2*pi*25*max(F_deNoise)/0.0245; 
% cov0 = .1e-3*diag([0, 1, 0, 1, 0, 0, 0]); % the initial state covariance
cov0 = 1e-4*diag([0, 3, 1, 0, 0, 0]); % the initial state covariance
...
% xlim([0 1]); ylim(100*[-1 1])

% xlim([0.15 0.5]); ylim(20*[-1 1])

% xlim([0 .1]);xticks(0:0.05:0.1);
% ylim([-250 250]); yticks(-200:100:200)
