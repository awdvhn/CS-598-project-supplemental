function dydtarbi=dydtarbi(t,y,N,kg1,kg2,d_g1,d_g2,d_c,d_NL1,d_NL2,m1,m2,coupling,kNL1,kNL2,kNL_L1,kNL_L2,beta,F_ham,Tspan1,loc)
%xi=0.005;
F=interp1(Tspan1,F_ham,t);
dydtarbi=zeros(4*N,1);
dydtarbi(1)=y(2);%dydt(4n+1)=dx/dt
dydtarbi(2)=loc(1)*F/m1(1)+(-kg1(1)*y(1)-kNL1(1)*abs(y(1)-y(5))^(beta-1)*(y(1)-y(5))-kNL_L1(1)*(y(1)-y(5))-coupling(1)*(y(1)-y(3))-d_g1(1)*y(2)-d_c(1)*(y(2)-y(4))-d_NL1(1)*(y(2)-y(6)))/m1(1);
dydtarbi(3)=y(4);%dydt(4n+3)=dy/dt
dydtarbi(4)=loc(2)*F/m2(1)+(-kg2(1)*y(3)-kNL2(1)*abs(y(3)-y(7))^(beta-1)*(y(3)-y(7))-kNL_L2(1)*(y(3)-y(7))-coupling(1)*(y(3)-y(1))-d_g2(1)*y(4)-d_c(1)*(y(4)-y(2))-d_NL2(1)*(y(4)-y(8)))/m2(1);
...
dydtarbi(4*N-2)=loc(4)*F/m1(N)+(-kg1(N)*y(4*N-3)-kNL1(N-1)*abs(y(4*N-3)-y(4*N-7))^(beta-1)*(y(4*N-3)-y(4*N-7))-...
...
dydtarbi(4*N)=loc(3)*F/m2(N)+(-kg2(N)*y(4*N-1)-kNL2(N-1)*abs(y(4*N-1)-y(4*N-5))^(beta-1)*(y(4*N-1)-y(4*N-5))-...
